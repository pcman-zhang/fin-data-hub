...
| `cn_equity.financials.balance_sheet` | cn_equity | 资产负债表（核心列；append-only 版本，按公告日知识时间） | entity | versioned | tushare | `get_financials(kind="balance_sheet")` |
...
| `ref.entity` | ref | 引用注册表（实体身份 + PIT 属性/生命周期；SCD2；instrument/series/basket） | entity | scd2 | — | `get_security_info / 内部` |
