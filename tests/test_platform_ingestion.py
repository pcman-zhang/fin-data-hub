...
class FakeHub:
    def __init__(self, rows: list[dict], source_attr: str | None = "akshare") -> None:
        self._rows = rows
...
def test_sync_appends_revision_when_values_change(engine) -> None:
    ...
    second = sync_daily_bar(
        engine, FakeHub(changed), code="600519.SH", start=DAY1, end=DAY2
    )
